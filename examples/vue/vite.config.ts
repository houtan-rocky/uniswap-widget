...
// Dev-server config for the Vue example. The proxies below stand in for the
// backend the widget expects: a host app must forward these paths (token search
// + Base RPC) so the widget's network calls succeed. In production this example
// uses the Vercel functions under ./api (see vercel.json).
export default defineConfig({
  plugins: [
    vue({
      template: {
        compilerOptions: {
          // AppKit registers <appkit-*> web components.
